...
export function TabletopControls({ grid, keyboardEnabled, preferArrows, gamerMode }: TabletopControlsProperties) {
  ...
  const [ready, setReady] = useState(grid.robotPosition !== undefined);
  // Update "ready" state object from observable
  useEffect(() => {
    ...
    <>
      ...
      <div className={styles.explicitPos}>
        <form onSubmit={onSubmit}>
          ...
          />
          <button type="submit">{t('coord-set')}</button>
        </form>
      </div>
    </>
  );
